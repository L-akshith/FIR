...
def get_active_risk_zones() -> list:
    ...
    result = (
        ...
        # Explicitly cast Geography columns to Text (WKT) 
        .select("id, disease, risk_level, case_count, radius_km, created_at, expires_at, centroid:centroid::text, boundary:boundary::text")
...
def get_historical_zones() -> list:
    ...
    # Explicitly cast Geography columns to Text (WKT) 
    result = client.table("historical_outbreaks").select("id, district, disease, radius_km, years, centroid:centroid::text").execute()
    return result.data or []


def save_risk_zones(zones: list[dict]) -> list:
    """
    Bulk replace risk zones in the risk_zones table.
    Deletes all existing zones then inserts the new batch.
    ...
    # Clear all existing zones first to avoid constraint violations
    try:
        client.table("risk_zones").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
        print(f"[Supabase] Cleared existing risk zones.")
    except Exception as e:
        print(f"[Supabase] Warning: Could not clear existing zones: {e}")
